...
var QueryBuilder = class {
  ...
  buildWhereCondition(column, operator, value) {
    switch (operator) {
      case "eq":
        return `${column} = ${(0, import_mysql2.escape)(value)}`;
      case "ne":
        return `${column} != ${(0, import_mysql2.escape)(value)}`;
      case "gt":
        return `${column} > ${(0, import_mysql2.escape)(value)}`;
      ...
      case "lte":
        return `${column} <= ${(0, import_mysql2.escape)(value)}`;
      case "in":
        return `${column} IN (${value.map(import_mysql2.escape).join(", ")})`;
      case "nin":
        return `${column} NOT IN (${value.map(import_mysql2.escape).join(", ")})`;
      case "like":
        return `${column} LIKE ${(0, import_mysql2.escape)(value)}`;
      default:
